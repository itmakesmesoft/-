...
const DonationList = () => {
  ...
  const {
    ...
    error,
    loading,
  } = useListApi("donation", pageNum, pageSize); // length는 페이지네이션 활용 용도 - 이은혁
  ...
  return (
    <>
          ...
          return (
            <div className="item" key={index}>
              ...
              <div className="item_sec">
                {data.projectCategory === "SQUIRREL" ? (
                  <img className="badge" src={badge_squirrel} alt="" />
                ) : (
                  ""
                )}
                {data.projectCategory === "CRANE" ? (
                  <img className="badge" src={badge_crane} alt="" />
                ) : (
                  ""
                )}
                {data.projectCategory === "WILD_ANIMAL" ? (
                  <img className="badge" src={badge_wild_animal} alt="" />
                ) : (
                  ""
                )}
              </div>
              <div className="item_sec main_sec">
                ...
                <p className="title mb-1">
                  <Link className="link" to={`/donation/` + data.projectId}>
                    {data.projectSubject}
                  </Link>
                </p>
                <p className="weak">
                  기부한 날짜 : {year + "." + month + "." + day}
                </p>
              </div>
              <div className="item_sec w-2 flex-column justify-center align-center">
                <p className="weak">내가 받은 티켓</p>
                <p className="strong flex-row justify-center align-center">
                  <HiOutlineTicket
                    className="icon"
                    color="#c2c2c3"
                    size="1.5rem"
      ...
      <p className="loading">
        {datas.length === 0 && !loading ? "기부한 내역이 없습니다." : ""}
        {loading ? "로딩 중" : ""}
      </p>
      <Paging
        page={pageNum}
